      ...
      <div className="surface-card w-full max-w-lg rounded-3xl p-6">
        <div className="mb-5 flex items-center justify-between">
          ...
          <button
            type="button"
            onClick={onClose}
            className="inline-flex h-9 w-9 items-center justify-center rounded-xl border border-[color:var(--line)] text-slate-800 transition hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800"
            aria-label="Close"
          >
        ...
        <form className="grid gap-4" onSubmit={handleSubmit}>
          <label className="grid gap-1">
            <span className="text-sm font-semibold text-slate-800 dark:text-slate-300">Date</span>
            <input
              type="date"
              value={form.date}
              onChange={(event) => handleChange("date", event.target.value)}
              className="h-11 rounded-xl border border-[color:var(--line)] bg-[color:var(--surface-strong)] px-3 text-slate-900 outline-none transition focus:border-cyan-400 dark:text-slate-100"
              required
            />
          </label>

          <label className="grid gap-1">
            <span className="text-sm font-semibold text-slate-800 dark:text-slate-300">Description / Note</span>
            <input
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="grid gap-1">
              <span className="text-sm font-semibold text-slate-800 dark:text-slate-300">Type</span>
              <select
                value={form.type}
            ...
            <label className="grid gap-1">
              <span className="text-sm font-semibold text-slate-800 dark:text-slate-300">Category</span>
              <select
                value={form.category}
          ...
          <label className="grid gap-1">
            <span className="text-sm font-semibold text-slate-800 dark:text-slate-300">Amount</span>
            <input
              type="number"
              min="1"
              value={form.amount}
